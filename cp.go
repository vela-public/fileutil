...
func (o Options) Copy(src, dst string) error {
	...
	if err := cp(src, dst, srcInfo); err != nil {
		return err
	}
...
// config.
func CopyTree(src, dst string, opt Options) error {
	return opt.CopyTree(src, dst)
}

func cp(src, dst string, srcInfo os.FileInfo) error {
	m := srcInfo.Mode()
	switch {
	case m.IsDir():
